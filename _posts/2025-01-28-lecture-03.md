...
**Generative Models** Observe X and Y, then learn $P(X \mid Y)$, $P(Y)$ and calculate $P(X) = \int P(X,Y) \, dY$. Then, use these to calcualate $P(Y\mid X) = \frac{P(Y, X)}{P(X)}$. 
...
**Discriminitive Models** Observe X and Y, and then learn $P(X\mid Y)$ It is a more straightforward approach, but you do not learn as much about X, Y and their distributions.
...
Our goal here is to observe X and Y and from then then learn $P(X\mid Y)$, $P(Y)$ and calculate $P(X) = \int P(X, Y) \, dY$. Then, use these to calcualate $P(Y\mid X) = \frac{P(Y, X)}{P(X)}$.
...
$\hat{\theta}_{\text{disc}} = \arg\max_{\theta} P(Y\mid X; \theta) = \arg\max_{\theta} \frac{P(X\mid Y; \theta)P(Y; \theta)}{P(X; \theta)}$
Generative Models optimize the *joint* likelihood:
$\hat{\theta}_{\text{gen}} = \arg\max_{\theta} P(X, Y; \theta) = \arg\max_{\theta}P(X\mid Y; \theta) P(Y; \theta)$
