...
\begin{figure}[h]				    (Put the figure)
  \includegraphics[width=0.5\linewidth]{image.png}  (Define the width of the figure)
  \caption{A sample figure}                         (A caption for the figure)
\end{figure}
```
### Tables
```latex
\begin{tabular}{|c|c|}   （Start table, defining 2 columns with centered text (c = center)）
